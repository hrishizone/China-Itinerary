  ...
  return (
    <section id="know" className="relative mx-auto max-w-7xl px-5 py-24 sm:px-8 sm:py-32">
      <SectionHeading
        ...
        lead="China is one of the easiest countries in the world to travel once you have solved three problems — the internet, the payments and the bookings. Solve them at home, in that order."
      />
              ...
              <Reveal key={e.id} delay={i * 0.05}>
                <div
                  className={`overflow-hidden rounded-xl border transition-colors ${
                    on ? 'border-vermilion-500/35 bg-vermilion-500/[0.06]' : 'border-white/8 bg-white/[0.02]'
                  }`}
                >
                  <button
                  ...
                  >
                    <span
                      className={`grid h-10 w-10 shrink-0 place-items-center rounded-lg transition-colors ${
                        on ? 'bg-vermilion-500/20 text-vermilion-300' : 'bg-white/5 text-white/50'
                      }`}
                    >
                      ...
                      >
                        <ul className="space-y-3 px-5 pb-5 pl-[4.5rem]">
                          {e.points.map((p, j) => (
                            ...
                            >
                              <span className="mt-[7px] h-1 w-1 shrink-0 rounded-full bg-vermilion-400" />
                              {p}
                            </motion.li>
                          ))}
                        </ul>
          ...
          <div className="lg:sticky lg:top-[100px]">
            <div className="rounded-2xl border border-white/8 bg-ink-900/70 p-6">
              <div className="mb-1 flex items-center justify-between">
                ...
                <span className="hanzi text-[13px] text-vermilion-300/60">普通话</span>
              </div>
              <p className="mb-5 text-[13px] leading-relaxed text-white/45">
                Genuinely enough to get through a week. Tap any card to hear it.
              </p>
              <div className="grid grid-cols-2 gap-2">
                {phrases.map((p, i) => (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.96 }}
                    className="group rounded-lg border border-white/8 bg-white/[0.03] p-3 text-left transition-colors hover:border-vermilion-500/35 hover:bg-vermilion-500/[0.08]"
                  >
                    <div className="flex items-start justify-between gap-1">
                      <span className="hanzi text-[17px] font-bold leading-tight text-white">{p.zh}</span>
                      <Volume2
                        size={12}
                        className="mt-1 shrink-0 text-white/20 transition-colors group-hover:text-vermilion-300"
                      />
                    </div>
                    <div className="mt-1 font-mono text-[10.5px] text-gold-300/80">{p.py}</div>
