    ...
    <div className={`mb-14 max-w-3xl ${align === 'center' ? 'mx-auto text-center' : ''}`} id={id}>
      <Reveal>
        <div className={`mb-4 flex items-center gap-3 ${align === 'center' ? 'justify-center' : ''}`}>
          <span className="h-px w-8 bg-vermilion-500" />
...
export function Chip({ children, tone = 'default', className = '' }) {
  const tones = {
    ...
    hot: 'border-vermilion-500/40 bg-vermilion-500/12 text-vermilion-200',
    gold: 'border-gold-400/40 bg-gold-400/12 text-gold-300',
    jade: 'border-jade-400/40 bg-jade-400/12 text-jade-300',
  }
  return (
    <span
...
export function toneForTag(tag) {
  ...
  const gold = ['Splurge', 'Worth it', 'Adventure', 'Photo', 'Hidden gem', 'Do it', 'Big move', 'Sleep saver']
  if (hot.includes(tag)) return 'hot'
  if (gold.includes(tag)) return 'gold'
  if (['Free', 'Cheap eat', 'Street food', 'Local'].includes(tag)) return 'jade'
  return 'default'
}
