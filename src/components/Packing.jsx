...
const KEY = 'dragons-trail:packed:v1'
...
export default function Packing() {
  ...
  return (
    <section className="relative mx-auto max-w-7xl px-5 pb-24 sm:px-8 sm:pb-32">
      <Reveal>
        <div className="overflow-hidden rounded-2xl border border-white/8 bg-gradient-to-br from-ink-900 to-ink-850">
          <div className="flex flex-wrap items-center justify-between gap-5 border-b border-white/8 p-6 sm:p-8">
            <div className="flex items-center gap-4">
              <span className="grid h-12 w-12 place-items-center rounded-xl bg-jade-500/12 text-jade-400">
                <Luggage size={22} />
              </span>
              <div>
          ...
          <div className="grid gap-px bg-white/8 sm:grid-cols-2 lg:grid-cols-4">
            {packing.map((g) => (
              <div key={g.group} className="bg-ink-900 p-6">
                <h4 className="mb-4 font-mono text-[10px] uppercase tracking-[0.22em] text-vermilion-300/80">
                  {g.group}
                </h4>
                          ...
                          <span
                            className={`mt-[1px] grid h-[18px] w-[18px] shrink-0 place-items-center rounded-[5px] border transition-all duration-300 ${
                              on
                                ...
                                : 'border-white/20 bg-transparent group-hover:border-jade-400/60'
                            }`}
                          >
                            <motion.span
                              initial={false}
