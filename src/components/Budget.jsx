  ...
  return (
    <section id="budget" className="relative mx-auto max-w-7xl px-5 py-24 sm:px-8 sm:py-32">
      <SectionHeading
        ...
        title={
          <>
            ₹1,00,000, <span className="gradient-text">accounted for</span>.
          </>
        }
        lead="Not an estimate — this is the sum of every line item in the seven days above, plus the fixed costs you pay before you leave home. Switch the style of travel and watch it move."
      />
            ...
            {Object.entries(budget.modes).map(([k, m]) => (
              <button
                ...
                className={`relative rounded-lg px-4 py-2 text-[13px] font-semibold transition-colors ${
                  mode === k ? 'text-ink-950' : 'text-white/55 hover:text-white'
                }`}
              >
                {mode === k && (
                  <motion.span
      ...
      <div className="grid gap-10 lg:grid-cols-[380px_1fr]">
        {/* ── Donut ─────────────────────────────────────── */}
        <Reveal>
          <div className="relative rounded-2xl border border-white/8 bg-ink-900/60 p-6">
                ...
                >
                  {shown ? (
                    ...
                    <>
                      ...
                      <div
                        className={`mt-1 text-[12px] font-medium ${over ? 'text-vermilion-300' : 'text-jade-400'}`}
                      >
                        {over
                          ? `${inr(total - budget.total)} over the lakh`
          ...
          <Reveal delay={0.2}>
            <div
              className={`mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border p-5 ${
                over
                  ? 'border-vermilion-500/35 bg-vermilion-500/10'
                  : 'border-jade-500/30 bg-jade-500/8'
              }`}
            >
              <div>
              ...
              <div className="text-right text-[13px] leading-relaxed text-white/60">
                {over ? (
                  <>
                    <strong className="text-vermilion-200">{inr(total - budget.total)} over</strong> the
                    ₹1,00,000 target.
                    <br />
                    Comfort mode is a ₹1.28 lakh trip — worth knowing.
                  </>
                ) : (
                  <>
                    <strong className="text-jade-300">{inr(budget.total - total)} under</strong> the
                    ₹1,00,000 target.
                    <br />
                    That is your emergency fund, not your shopping money.
                  </>
                )}
              </div>
            </div>
      ...
      <div className="mt-20">
        <Reveal>
          <h3 className="heading mb-2 text-2xl sm:text-3xl">
            Six decisions that make a lakh work
          </h3>
          <p className="mb-8 max-w-2xl text-[15px] leading-relaxed text-white/50">
            None of these are about going without. They are about the structure of the trip — and together
            they are the difference between this being possible and not.
          </p>
        </Reveal>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {savings.map((s, i) => (
            <Reveal key={s.title} delay={i * 0.07}>
              <div className="card-hover h-full rounded-xl border border-white/8 bg-ink-900/60 p-5">
                <div className="mb-3 flex items-center justify-between">
                  <span className="grid h-9 w-9 place-items-center rounded-lg bg-jade-500/12 text-jade-400">
                    <Icon name={s.icon} size={16} />
                  </span>
                  <span className="flex items-center gap-1 font-mono text-[12px] font-semibold text-jade-400">
